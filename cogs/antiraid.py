...
def antiraid_trusted():
    """Check que verifica si el usuario es owner o está en la lista de trusted (unificada con Antinuke)"""
    async def predicate(ctx: commands.Context) -> bool:
        ...
        # Verificar si está en la lista de trusted (unificada con Antinuke)
...
class Antiraid(commands.Cog):
    ...
    async def is_trusted(self, guild_id: int, user_id: int) -> bool:
        ...
        # Trusted del antiraid
        settings = await self.get_settings(guild_id)
        if settings:
            trusted.update(settings.get("trusted", []))

        # Trusted del antinuke
        try:
            an_doc = await database.antinuke_servers.find_one({"guild_id": guild_id}, {"trusted": 1})
            if an_doc:
                trusted.update(an_doc.get("trusted", []))
        except Exception:
            pass
    ...
    async def antiraid_penalty(self, ctx: commands.Context, action: Literal["ban", "kick", "quarantine"]):
        ...
        **Uso:** ;antiraid penalty <ban/kick/quarantine>
        ...
        # INVALIDAR caches correctamente (antes tardaba en actualizar)
        await self.invalidate_cache(ctx.guild.id)
    ...
    async def antiraid_settings(self, ctx: commands.Context):
        ...
        embed.add_field(
            ...
            value=f"`{ctx.prefix}antiraid enable` - Habilitar antiraid\n"
                  ...
                  f"`{ctx.prefix}antiraid whitelist` - Gestionar whitelist (Antinuke)",
            inline=False
        )
    ...
    async def trusted(self, ctx: commands.Context):
        """Ver los usuarios de confianza que pueden configurar el antiraid (y Antinuke)"""
        settings = await self.get_settings(ctx.guild.id)
        trusted_users = settings.get("trusted", []) if settings else []

        # Unificar con antinuke
        try:
            an_doc = await database.antinuke_servers.find_one({"guild_id": ctx.guild.id}, {"trusted": 1})
            if an_doc:
                for uid in an_doc.get("trusted", []):
        ...
        embed = discord.Embed(
            ...
            description="\n".join(lines) + "\n\n*Estos usuarios pueden configurar el antiraid y el antinuke*",
            color=config.BLURPLE_COLOR
        )
    ...
    async def trusted_add(self, ctx: commands.Context, user: discord.User):
        """Añadir usuario trusted (solo owner) — Unificado con Antinuke"""
        if ctx.author.id != ctx.guild.owner_id:
            return await ctx.send(embed=error_embed(
        ...
        # INVALIDAR caches antiraid correctamente (bug de demora)
        await self.invalidate_cache(ctx.guild.id)
        ...
        if antinuke_cog:
            try:
                await antinuke_cog.invalidate_cache(ctx.guild.id)
            except Exception:
                pass

        embed = success_embed(f"**{user}** ahora puede configurar el antiraid y el antinuke", ctx.author)
        await ctx.send(embed=embed)

    @trusted.command(name="remove", aliases=["quitar", "del"])
    async def trusted_remove(self, ctx: commands.Context, user: discord.User):
        """Quitar usuario trusted (solo owner) — Unificado con Antinuke"""
        if ctx.author.id != ctx.guild.owner_id:
            return await ctx.send(embed=error_embed(
        ...
        embed = success_embed(f"**{user}** ya no puede configurar el antiraid ni el antinuke", ctx.author)
        await ctx.send(embed=embed)

    # ========== Whitelist (UNIFICADA con Antinuke) ==========

    @antiraid.group(name="whitelist", aliases=["wl"], invoke_without_command=True)
    @antiraid_trusted()
    async def whitelist(self, ctx: commands.Context):
        """Ver la whitelist (unificada con Antinuke)"""
        # Reutilizar directamente la colección de antinuke
    ...
    async def whitelist_add(self, ctx: commands.Context, user: discord.User):
        """Añadir usuario a la whitelist (Antinuke)"""
        ...
        # Invalidar caches (Antinuke whitelist + Antiraid cache)
        try:
    ...
    async def whitelist_remove(self, ctx: commands.Context, user: discord.User):
        """Quitar usuario de la whitelist (Antinuke)"""
...
class AntiraidSettingsView(discord.ui.View):
    ...
    async def toggle_enabled(self, interaction: discord.Interaction, button: discord.ui.Button):
        """Toggle antiraid on/off (owner o trusted)"""
        ...
        await self.cog.invalidate_cache(self.ctx.guild.id)
...
class AntiraidPenaltySelect(discord.ui.Select):
    ...
    async def callback(self, interaction: discord.Interaction):
        ...
        await self.parent_view.cog.invalidate_cache(self.parent_view.ctx.guild.id)
